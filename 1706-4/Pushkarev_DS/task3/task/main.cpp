...
int* parallel(int* G, int start, int countVertex)
{
	...
	int* countElementG = new int[procNum];//количество элементов передаваемых каждому процессу[rank]
	...
	//int countEdge = (countVertex - 1) + rand() % ((countVertex * (countVertex - 1)) / 2);//слева минимум, справа
	int* Graph = new int[countVertex * countVertex];
	if (rank == 0)
	{
	...
	while (!q.empty())
	{
		...
		/*if (dest > d[v])
			...
			//for (int j = v * countVertex; j < v * countVertex + countVertex; j++)//диапазон вершины
