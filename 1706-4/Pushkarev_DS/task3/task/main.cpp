...
int* initOneG(int** G, int countEdge, int countVertex)//создает из матрицы массив
{
...
int** iniG(int countEdge, int countVertex)//создает матрицу
{
	//cout << "Matrix" << endl;
...
int* manually(int countVertex)//задаем вручную
{
	cout << "Matrix" << endl;
	int rank = 0, procNum = 0;
	MPI_Comm_size(MPI_COMM_WORLD, &procNum);
...
int* parallel(int* G, int start, int countVertex, vector<vector<int>>& vec)
{
	...
	if (countElementG[procNum - 1] > countElementG[1])
	{
		...
		//cout <<"RAZN "<< razn << endl;																	//(количество вершин, на сколько больше в посл, чем в первом
		...
		//countElementD[procNum - 1] -= razn;//теперь у него меньше вершин
	}
	for (int i = 1; i < procNum; i++)
	{
		//distG[i] = i * countElementG[i - 1];//начало отсчета из массива G для каждого процесса
	...
	if (rank == 0)
	{
		...
		while (!q.empty())
		{
			...
			for (int i = 1; i < procNum; i++) {
				...
				flagRank0 = 1;
				//cout << "rank = " << i << endl;
				while (flagRank0 != 0) {//если == 0 значит !0 процесс завершил рассмотрение
					//cout << "rank = 0" << endl;
		...
		while (flag) {
			...
			for (int j = 0; j < countElementD[rank]; j++)
			{
				...
				//v = countVertex % (countElementG[rank] / countVertex);//меняем на локальный номер вершины
...
int main(int argc, char* argv[])
{
	...
	MPI_Init(&argc, &argv);
	countVertex = atoi(argv[1]);////////////ВКЛЮЧИТЬ
	//countVertex = 8;
	//countVertex = 5;
	//srand(time(NULL));
